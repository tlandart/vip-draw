...
export async function createHost(hostId) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/create-host`, {
      method: "POST",
...
export async function deleteGame(hostId) {
  try {
    const response = await fetch(`${BASE_URL}/delete-game/${hostId}`, {
      method: "DELETE",
    });
    if (!response.ok) {
...
export async function checkGame(hostId) {
  try {
    const response = await fetch(`${BASE_URL}/get-game/${hostId}`, {
      method: "GET",
    });

    if (!response.ok) {
      if (response.status === 404) {
        return { error: "Game ID does not exist. Please check the ID and try again." };
      }
